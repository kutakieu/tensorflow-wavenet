...
class TestNet(tf.test.TestCase):
    ...
    def testEndToEndTraining(self):
        ...
        with self.test_session() as sess:
            ...
            val_lc = np.pad(val_lc, ((0, 0), (receptive_field - 1, 0)), 'constant')
            sample_period = 1.0 / SAMPLE_RATE_HZ
            times = np.arange(0.0, SAMPLE_DURATION, sample_period)
            note1 = 0.6 * np.sin(times * 2.0 * np.pi * F1)
            note2 = 0.5 * np.sin(times * 2.0 * np.pi * F2)
            ...
            for i in range(self.train_iters):
                ...
                if i % 10 == 0:
                    ...
                    for i in range(3):
                        ...
                        validation_score += sess.run(validation, feed_dict={audio_placeholder: val_audio[:,300*i:300*(i+1)+receptive_field],
                                                                            lc_placeholder: val_lc[:,300*i:300*(i+1)+receptive_field],
